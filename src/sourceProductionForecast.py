...
def initialize(inFileName, localTimezone):
    ...
    # return dataset, dateTime, None, None
...
for ISO in ISO_LIST:
    ...
    IN_FILE_NAME = "../data/"+ISO+"/fuel_forecast/"+ISO+"_"+FUEL[START_COL]+"_2019_clean.csv"
    OUT_FILE_NAME_PREFIX = "../data/"+ISO+"/fuel_forecast/"+ISO+"_LSTM_"+str(PREDICTION_WINDOW_HOURS)+"_hr_src_prod_forecast"
    if(FORECASTS_ONE_DAY_AT_A_TIME is True):
    ...
    for period in range(4):
        ...
        for xx in range(NUMBER_OF_EXPERIMENTS):
            # OUT_FILE_SUFFIX = "_iter"+str(xx+1)
            OUT_FILE_NAME = OUT_FILE_NAME_PREFIX + "_" + featureList[0] + OUT_FILE_SUFFIX + ".csv"
